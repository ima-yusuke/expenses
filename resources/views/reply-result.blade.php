    ...
    <div class="min-h-screen bg-gradient-to-br from-primary-50 via-white to-accent-50">
        <section class="py-16 px-4 sm:px-6 lg:px-8">
            <div class="max-w-4xl mx-auto">
                <div class="mb-10">
                    <a href="{{route('ShowReplyAssistant')}}" class="inline-flex items-center text-sm text-primary-600 hover:text-primary-900 font-medium transition-colors">
                ...
                <div class="space-y-6">
                    <!-- 友達のメッセージ -->
                    <div class="bg-white/80 backdrop-blur-sm border border-primary-100 rounded-2xl p-8 shadow-soft">
                        <div class="flex items-start">
                            ...
                            <div class="ml-4 flex-1">
                                <h3 class="text-sm font-semibold text-primary-700 mb-3">友達からのメッセージ</h3>
                                <p class="text-primary-900 leading-relaxed">{{$friendMessage}}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white/80 backdrop-blur-sm border border-primary-100 rounded-2xl p-8 shadow-soft">
                        <div class="flex items-start">
                            <div class="flex-shrink-0">
                                <div class="w-10 h-10 rounded-full bg-gradient-to-br from-accent-100 to-accent-200 flex items-center justify-center">
                            ...
                            <div class="ml-4 flex-1">
                                <h3 class="text-sm font-semibold text-accent-700 mb-3">あなたの返信意図</h3>
                                <p class="text-primary-900 leading-relaxed">{{$replyIntent}}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gradient-to-br from-white to-accent-50/30 backdrop-blur-sm border-2 border-accent-200 rounded-2xl p-6 sm:p-8 shadow-soft-lg">
                        ...
                        <!-- 英文のみ -->
                        <div class="bg-white/90 backdrop-blur-sm border border-accent-200 rounded-xl p-6 mb-6 shadow-inner-soft">
                            <div class="flex items-start mb-3">
                                <span class="inline-block px-3 py-1 bg-accent-100 text-accent-800 text-xs font-semibold rounded-full">英語返信</span>
                            </div>
                            <p id="english-reply" class="text-lg text-primary-900 leading-relaxed font-medium">{{$englishReply ?? ''}}</p>
                        </div>
                    ...
                    @if(count($usedWords) > 0)
                        <div class="bg-white/80 backdrop-blur-sm border border-primary-100 rounded-2xl p-8 shadow-soft">
                            <h3 class="text-sm font-semibold text-primary-900 mb-6 flex items-center">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach($usedWords as $word)
                                    <div class="bg-gradient-to-br from-white to-primary-50 border border-primary-200 rounded-xl p-5 hover:shadow-soft transition-all duration-300">
                                        <p class="font-bold text-primary-900 text-lg mb-2">{{$word->word}}</p>
                                        <div class="space-y-1">
                                            @foreach($word->japanese as $ja_word)
                                                <p class="text-sm text-primary-700">・{{$ja_word->japanese}}</p>
                                            @endforeach
                                        </div>
                                    </div>
                                @endforeach
